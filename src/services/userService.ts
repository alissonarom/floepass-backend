...
export const createOrUpdateUser = async (
  ...
  const filter = userId ? { _id: userId, client_id } : { cpf, client_id };
  ...
  const { createdAt, updatedAt, ...cleanUserData } = restUserData as any;
  ...
  if (cpf && !filter.cpf) {
    updateOperation.$setOnInsert.cpf = cpf;
  }

  try {
    const result = await db
      .collection("users")
      .findOneAndUpdate(filter, updateOperation, updateOptions);
